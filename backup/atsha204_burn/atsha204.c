...
#define WAKE_UP_BIT		1<<0
#define WRITE_CFG_BIT	1<<1
#define LOCK_CFG_BIT	1<<2
#define WRITE_OTP_BIT   1<<3
#define WRITE_DATA_BIT  1<<4
#define LOCK_DATA_BIT   1<<5
...
static uint32_t atsha204_wakeup(struct i2c_client *client, u8 *buf, u32 num);
...
static void atsha204_read_cfg_all(void);

uint8_t Buffer1[39] = {0x27,0x12,0x82,0x00,0x00,0x11,0x11,0x23,0xB6,0xCC,0x53,0xB7,0xB9,0xE9,0xBB,0x51,0xFD,0x2F,0x74,0xCD,0x0E,
												0x91,0xD9,0x7F,0xEB,0x84,0x7B,0x98,0x09,0xF4,0xCD,0x93,0x6A,0xB6,0x48,0x11,0x11,0x00,0x00};
uint8_t Buffer2[39] = {0x27,0x12,0x82,0x08,0x00,0x11,0x11,0x23,0xB6,0xCC,0x53,0xB7,0xB9,0xE9,0xBB,0x51,0xFD,0x2F,0x74,0xCD,0x0E,
												0x91,0xD9,0x7F,0xEB,0x84,0x7B,0x98,0x09,0xF4,0xCD,0x93,0x6A,0xB6,0x48,0x11,0x11,0x00,0x00};	
uint8_t Buffer3[39] = {0x27,0x12,0x82,0x10,0x00,0x22,0x22,0xC1,0x7C,0x1C,0x4D,0x56,0x89,0xAA,0x00,0x43,0xE3,0x9C,0xFB,0x6B,0x0B,
												0x68,0x49,0xE3,0x2C,0x24,0xA4,0x1B,0x06,0x34,0x49,0x1E,0x90,0x6B,0x62,0x22,0x22,0x00,0x00};
uint8_t Buffer4[39] = {0x27,0x12,0x82,0x18,0x00,0xc1,0x25,0x35,0xd4,0x85,0x74,0x45,0x54,0x64,0x71,0x14,0x19,0xb4,0xa5,0xff,0x54,
												0x74,0xf9,0x2b,0xb7,0xa4,0x61,0x7f,0xa3,0x32,0x12,0x28,0x76,0xc9,0x84,0x46,0x3f,0x00,0x00};
uint8_t Buffer5[39] = {0x27,0x12,0x82,0x20,0x00,0xa5,0xc8,0x02,0x10,0x54,0x40,0xfb,0x41,0xa6,0x22,0xa7,0x16,0x84,0x76,0x00,0x43,
												0x31,0xb7,0xaa,0x45,0x73,0x92,0x17,0x93,0x13,0x73,0x18,0x43,0x43,0x18,0x16,0x67,0x00,0x00};
uint8_t Buffer6[39] = {0x27,0x12,0x82,0x28,0x00,0x45,0x87,0x46,0x92,0x20,0xb6,0x46,0x16,0xf5,0x56,0x96,0xfc,0x13,0xa6,0xa6,0x73,
												0xb9,0x84,0x71,0x43,0x38,0x16,0x56,0xd9,0xe9,0xe4,0xa6,0xff,0x41,0xee,0xa6,0xf8,0x00,0x00};
uint8_t Buffer7[39] = {0x27,0x12,0x82,0x30,0x00,0xd6,0x23,0x45,0x12,0x56,0x89,0x74,0x89,0x46,0x24,0x63,0x71,0x46,0x16,0x75,0x89,
												0xd5,0x13,0xe9,0x76,0x14,0x53,0x42,0x89,0x82,0x46,0x13,0xd5,0x11,0x25,0x69,0x46,0x00,0x00};
uint8_t Buffer8[39] = {0x27,0x12,0x82,0x38,0x00,0xc3,0xfb,0x39,0x47,0x56,0x84,0x14,0x26,0x84,0x43,0x49,0x11,0x44,0x13,0x34,0x31,
												0x98,0x35,0x34,0x11,0x47,0x36,0x44,0x36,0x47,0x62,0x34,0x74,0xd5,0xe9,0xac,0xfd,0x00,0x00};
uint8_t Buffer9[39] = {0x27,0x12,0x82,0x40,0x00,0x46,0x48,0x14,0xca,0x15,0x78,0x91,0x68,0xc6,0x4f,0xc7,0xa6,0x14,0x22,0x47,0xdc,
												0x63,0x34,0x52,0x18,0xf4,0x7d,0x12,0x00,0x1f,0xaa,0x13,0x31,0x25,0x19,0x67,0x94,0x00,0x00};
uint8_t Buffer10[39] ={0x27,0x12,0x82,0x48,0x00,0xce,0xe3,0x24,0xa6,0x18,0xf6,0x66,0x44,0xa5,0xc6,0xd8,0xcc,0x77,0xe9,0xf6,0xd5,
												0x5f,0x2a,0x36,0x71,0x24,0x46,0xfc,0xaa,0x12,0xc6,0x13,0xa9,0x16,0xff,0x69,0x82,0x00,0x00};
uint8_t Buffer11[39] ={0x27,0x12,0x82,0x50,0x00,0x45,0x95,0xd6,0xf6,0x21,0x71,0x24,0xff,0x22,0x22,0x22,0xa3,0x75,0xf6,0x24,0xd6,
												0xa3,0xb6,0x84,0x78,0x89,0x34,0xd6,0x54,0x5f,0x2a,0x8d,0xe5,0x6e,0x22,0x33,0x46,0x00,0x00};
uint8_t Buffer12[39] ={0x27,0x12,0x82,0x58,0x00,0x13,0x74,0xef,0xfc,0x1a,0x51,0x61,0x38,0x94,0x2b,0x7b,0x56,0x69,0x43,0x27,0x73,
												0x81,0xa6,0x12,0xf6,0xd2,0x21,0xcb,0xbf,0xcd,0xcf,0xad,0x00,0xff,0x46,0x33,0x44,0x00,0x00};
uint8_t Buffer13[39] ={0x27,0x12,0x82,0x60,0x00,0x56,0xd4,0x32,0xac,0xcd,0x56,0x77,0xd9,0x46,0x17,0xed,0xd6,0xac,0x13,0x74,0x58,
												0x13,0xed,0x13,0xd6,0x11,0x78,0x98,0x58,0x68,0x41,0x28,0xdc,0xcd,0xca,0x13,0x12,0x00,0x00};
uint8_t Buffer14[39] ={0x27,0x12,0x82,0x68,0x00,0x46,0x78,0xaa,0x56,0xfc,0xed,0xef,0x2e,0x3e,0x16,0x34,0xaa,0xcd,0xac,0x83,0x17,
												0x16,0x54,0x83,0x27,0x19,0x94,0x67,0x49,0x35,0x44,0x87,0x53,0x46,0x24,0x35,0x68,0x00,0x00};
uint8_t Buffer15[39] ={0x27,0x12,0x82,0x70,0x00,0xd6,0xa3,0xd2,0xe6,0xa2,0xc6,0x34,0x24,0x86,0x98,0x37,0xdd,0x3a,0x2c,0x14,0xd6,
												0x2c,0x7f,0xee,0xff,0x3a,0x68,0x10,0x00,0x00,0xad,0x43,0x2e,0xd7,0xd3,0x24,0xa6,0x00,0x00};
uint8_t Buffer16[39] ={0x27,0x12,0x82,0x78,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
...
static int atsha204_i2c_rx_data(char *buf, int len)
{
	uint8_t i;
	struct i2c_msg msgs[] = {
		{
...
static int atsha204_i2c_tx_data(char *buf, int len)
{
	uint8_t i;
	struct i2c_msg msg[] = {
		{
...
{
	pr_err("atsha204 open \n");
	return nonseekable_open(inode, file);
}

static ssize_t atsha204_read (struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	int ret = 0;
	uint8_t wake_buf[4] = {0};

	pr_err("atsha204 read here \n");
	ret = atsha204_wakeup(Client, wake_buf, 1);
	if(ret < 0)
	{
		pr_err("atsha204 wakeup error \n");
		return -1;
	}	
	ret = atsha204_mac(Client);

	return ret;
}

static ssize_t atsha204_write (struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	int ret = 0;
	int err_code = 0;
	uint8_t wake_buf[4] = {0};

	pr_err("atsha204 write here \n");
	ret = atsha204_wakeup(Client, wake_buf, 1);
	
	if(ret < 0)
	{
		return -WAKE_UP_BIT;
		pr_err("atsha204 write wakup error \n");
	}	

	if(atsha204_write_cfg(Client) == 8){
		atsh204_dbg(">>>>>> atsha204 write cfg success\n");
	}else{
		atsh204_err(">>>>>> atsha204 write cfg fail\n");
		err_code = WRITE_CFG_BIT;
	}
	...
	if(atsha204_lock_cfg(Client) != -1){
		...
		atsh204_err(">>>>>> atsha204 lock cfg fail\n");
		err_code |= LOCK_CFG_BIT;
	}
	...
	if(atsha204_write_otp(Client) == 7){
		...
		atsh204_err(">>>>>> atsha204 write otp fail\n");
		err_code |= WRITE_OTP_BIT;
	}
	...
	if(atsha204_write_data(Client) == 6){
		...
		atsh204_err(">>>>>> atsha204 write data fail\n");
		err_code |= WRITE_OTP_BIT;
	}
	...
	if(atsha204_lock_otp_data(Client) != -1){
		...
		atsh204_err(">>>>>> atsha204 lock otp fail\n");
		err_code |= LOCK_DATA_BIT;
	}
	
	if(err_code)
	{
		atsha204_read_cfg_all();
		ret = - err_code;
	}	
	else
	{
...
static int atsha204_release(struct inode *inode, struct file *file)
{
	pr_err("atsha204 release \n");
	return 0;
}

static long atsha204_ioctl(struct file *file,
	unsigned int cmd, unsigned long arg)
{
	...
	case 0:
		data[0] = 0;
		data[1] = 1;
		if(atsha204_i2c_tx_data(data, 2) < 0) {
			return -EFAULT;
		}
		break;
	case 1:
		data[0] = 0;
		if(atsha204_i2c_rx_data(data, 2) < 0) {
			return -EFAULT;
		}
		vec[0] = (int)data[0];
		vec[1] = (int)data[1];
...
//	memcpy(MsgBuf,Buffer3,32);
	for(i = 0;i < 32;i++)
		printk("%d: ",rand_buf[i]);
	//MsgBuf[32] = rand_buf[2];
	//buffer1[5] = rand_buf[2];
...
static struct file_operations atsha204_fops = {
       .owner			= THIS_MODULE,
       .open			= atsha204_open,
	   .write			= atsha204_write,
	   .read			= atsha204_read,
       .release			= atsha204_release,
       .unlocked_ioctl	= atsha204_ioctl,
};

static struct miscdevice atsha204_device = {
...
static int __devinit atsha204_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = misc_register(&atsha204_device);
	if (ret) {
		pr_err("%s: atsha204_device register failed\n", __FUNCTION__);
		goto out_deregister;
	}
...
static int atsha204_remove(struct i2c_client *client)
{
	atsh204_err(">>>>>> atsha204 remove\n");
	misc_deregister(&atsha204_device);
	kfree(sha204_st);
...
static struct i2c_driver atsha204_driver = {
	...
	.remove		= __devexit_p(atsha204_remove),
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Atsha204 Crypto driver");
MODULE_AUTHOR("Cong.he<hecong129@gmal.com>");
